...
class MuraImageLoader:
    img_ext     = '.bmp'
    muradata384 = 'mesdata384'
    muradata640 = 'mesdata640'
    dstStartRow = 2
    img_quality = 95
        ...
        for root, _, files in os.walk(self._img_folder):
            for file in files:
                if os.path.splitext(file)[-1] == self.img_ext:
                    ...
                    if self.muradata384 in file.lower():
                        self._384.append(filepath)
                    if self.muradata640 in file.lower():
                        self._640.append(filepath)
    ...
    def _export(self)->None:
        ...
        with open_workbook(self._xl_template, self._dst_xl) as wb:
            ws = wb.worksheets[0]
            if len(self._384) == len(self._640) and len(self._384) > 0:
                ubound = len(self._384)
                for k in range(0, ubound):
                    img_384 = self._384[k]
                    img_640 = self._640[k]
                    if not self._is_pmod_id_match(img_384, img_640):
                        logging.info("PmodIDNotMatchError")
                        break
                    img = Image.open(img_384)
                    ...
                    ws[dstCellAddress].value = os.path.split(img_384)[-1]
                    dstCellAddress = f'{self.dstCellCol384}{i}'
                    ws.add_image(img, dstCellAddress)
                    i += 1

                    img = Image.open(img_640)
                    width_percent = (self.width/float(img.size[idx_w]))
                    hsize = int((float(img.size[idx_h])*float(width_percent)))
                    img = img.resize((self.width, hsize), Image.ANTIALIAS)
